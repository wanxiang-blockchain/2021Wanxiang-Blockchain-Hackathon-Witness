 ...
 */
contract Controller is ControllerStorage, OwnableUpgradeable {
    ...
    event VotePublic(uint256 indexed id);

    /**
     * Update IPFS hash to tracking the last public voting entry
     */
    function votePublic(uint256 id, string memory vote) public onlyValidator {
        publicVotes[id] = vote;

        emit VotePublic(id);
    }

    mapping(uint256 => string) public publicVotes; // IPFS hash string of the voting data
}
